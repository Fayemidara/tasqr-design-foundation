...
export function Step6Review({
  ...
  const [prefix, setPrefix] = useState<string | null>(apiKeyPrefix);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data) {
        ...
        if (!prefix && data.api_key_prefix) setPrefix(data.api_key_prefix);
      }
    })();
  ...
  const handlePublish = async () => {
    ...
    const { error: profErr } = await supabase
      .from("profiles")
      .update({ role: "both", is_seller_onboarded: true })
      .eq("id", user.id);
  ...
  const maskedKey = prefix ? `${prefix}••••••••••••` : "Not generated";
  ...
  return (
    <>
      ...
      <Section title="Seller Profile" onEdit={() => onEdit(1)}>
        <Row label="Handle" value={`@${step1.handle}`} />
        <Row label="Display name" value={step1.displayName} />
        <Row label="Bio" value={step1.bio} />
        {step1.website && <Row label="Website" value={step1.website} />}
      </Section>

      <Section title="API Key" onEdit={() => onEdit(2)}>
        <div className="font-mono text-sm" style={{ color: "#FFD600" }}>
          {maskedKey}
        </div>
        <p className="mt-2 font-sans text-xs text-muted-foreground">
          Your full key was shown once during setup. If you need a new key, you can
          rotate it from your dashboard.
        </p>
      </Section>

      <Section title="Input Fields" onEdit={() => onEdit(3)}>
      ...
      <Section title="Agent Connection" onEdit={() => onEdit(4)}>
      ...
      <Section title="Listing Details" onEdit={() => onEdit(5)}>
