...
export function makeField(id: string): BuilderField {
  return {
    id,
    label: "",
    type: "text",
...
export function fieldError(f: BuilderField): string | null {
...
export function fieldsFromSchema(
  ...
  startId: (n: number) => string,
): BuilderField[] {
  if (!Array.isArray(schema) || schema.length === 0) return [];
  return schema.map((raw, i) => {
    const opts = Array.isArray(raw.options) ? (raw.options as unknown[]).map(String) : [];
    const t = String(raw.type ?? "text") as FieldType;
    return {
      id: startId(i),
      label: String(raw.label ?? ""),
      type: (["text", "textarea", "dropdown", "image_upload", "document_upload"].includes(t)
        ? t
        : "text") as FieldType,
...
export function useFieldIdFactory() {
  const nextIdRef = useRef(1);
  const nextId = () => `f_${nextIdRef.current++}`;
  const seed = (n: number) => {
    nextIdRef.current = Math.max(nextIdRef.current, n + 1);
    return `f_${n}`;
  };
  return { nextId, seed };
}
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_400px]">
      <div>
        ...
        {fields.length === 0 && (
          <p className="font-mono text-xs text-destructive mt-3">Add at least one field</p>
        )}
      </div>
      ...
      <div className="flex-1 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            ...
            <Input
              placeholder="e.g. Enter your text here, Paste your URL here, Upload your file here"
              value={field.label}
              onChange={(e) => onChange({ label: e.target.value })}
            />
            <p className="mt-1 font-mono text-[11px] text-muted-foreground">
              Field name: {toFieldName(field.label) || `field_${field.id.slice(0, 4)}`}
            </p>
          </div>
          <button
            type="button"
        ...
        {!hidePlaceholder && (
          <div>
            ...
            <Input
              placeholder="e.g. Describe what the buyer should provide in this field"
    ...
    <div>
      ...
      {(field.type === "image_upload" || field.type === "document_upload") && (
        <div className="border border-dashed border-border rounded-[4px] p-4 text-center font-sans text-xs text-muted-foreground">
          {field.type === "image_upload" ? "Upload an image" : "Upload a document"}
        </div>
      )}
    </div>
  );
