...
ALTER TABLE public.agents ADD COLUMN IF NOT EXISTS restored_at timestamptz;
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM public.disputes d WHERE d.run_id = r.id AND d.created_at > _window_start))
